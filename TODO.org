...
- [X] Prepare backlog
- [-] Replace local back-end (file implementation) with a RAM database [80%]
  ...
  - [X] Native nothing to do - Serialize database when org-trello is shut down (per buffer)
  - [X] Native nothing to do - Deserialize database when org-trello is started and merge with the existing one
  - [ ] Clean all the current running actions
  - [ ] Clean the current running action
