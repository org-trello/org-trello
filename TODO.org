...
- [-] Fix sync-from-trello
  - [X] Merge org card's users assigned list and the trello one - reference github issue
  - [ ] if new entities are referenced but not yet sync'ed on trello, they will disappear from the buffer when sync-from-trello (they should not) - reference github issue
- [ ] Release notes
* TODO 0.2.4 [0%]
