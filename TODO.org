...
** TODO Abstract away the implementation of the org-data some more (access of the information from the map are embedded in the function instead of delegating to dedicated function, thus not hiding the implementation)
** TODO Abstract away the query implementation too
** TODO Order when synchronizing from trello does not match trello's order
** TODO Migrate carton to card.el (travis-ci breaks because of this)
** TODO Introducing C-c o h at the loading of the buffer when ot is loaded
** TODO Implement the org-mode's standard checklist http://orgmode.org/manual/Checkboxes.html
...
** TODO Testing the packaging, at the moment, this is done manually (make package & M-x package-install-file)
* Questions
