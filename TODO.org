#+TITLE: orgtrello development progression
...
- [ ] Ensure packaging is generated and ok for melpa (is this possible?)
  - [ ] Remove org-trello from Melpa? (too much deps interference + no hand on packaging)
- [ ] Rewrite the abstraction around fetched result to unify the `data model` (in and out identical)
...
- [ ] Update the literate org-trello.org (completely desynchronized since 0.1.2) -> coding it to avoid doing manual split (I see the irony)
