...
- [ ] Fake trello server to help in testing.
- [ ] POC: use a temporary buffer to sync with trello, when done, merge with the user's buffer.
- [ ] POC: Investigate webhooks - #137
- [ ] POC: Leverage elnode and clojurescript core.async - #97

* IN-PROGRESS 0.5.2 [33%]
- [-] Is org-trello over-engineered? -> Seems so. [95%]
  ...
  - [X] Clean tests up
  - [X] Add deferred.el dependency
  - [X] Update org-trello package
  - [ ] Delete/Sync action as deferred computations
- [X] Clean README-dev.md about marmalade
- [X] Update dependencies version
- [ ] Now that elnode dependency's out of the way, org-trello can be fully built on melpa-stable.
- [ ] Update version
- [ ] Release notes
* DONE 0.5.1 [100%]
