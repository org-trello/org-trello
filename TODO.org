...
- [X] Improve the justify policy (trim the content before computing the justifying)
...
- [X] Check the scan problem with level 3 (must have been introduced with the refactoring from number to variable level)
- [ ] Improve map-checkbox
* TODO 0.1.8 [%]
