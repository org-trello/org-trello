...
* IN-PROGRESS Development steps [80%]
...
*** IN-PROGRESS Trigger the creation of the card and every level up to 3 (checklist + task)
** TODO Improve the callback (at the moment, the id is written where the carret is, so if we move while a http request takes place, the id is written badly)
** TODO Delete [0%]
*** TODO card
*** TODO checklist
*** TODO task
