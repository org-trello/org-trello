...
- [ ] Split the last org-trello namespace into 2
- [ ] Unfold every entries before triggering the sync from trello to avoid problems
- [ ] Rewrite the abstraction around fetched results to unify the `data model` (in and out identical)
