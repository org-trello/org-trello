...
- [X] Fix sync-from-trello - Merge org card's users assigned list and the trello one - reference github issue
- [-] Fix sync-from-trello - if new entities are referenced but not yet sync'ed on trello, they will disappear from the buffer when sync-from-trello (they should not) - reference github issue
...
- [ ] Upgrade version
- [ ] More fined grained use of the trello api (retrieve only data we use) - reference github issue
- [ ] Release notes
* TODO 0.2.4 [0%]
- [ ] More fined grained use of the trello api (retrieve only data we use)
