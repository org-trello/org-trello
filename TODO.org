...
- [ ] Again improve the justify policy, it is irritating at the moment (this moves to every align command executed and moving forward every time, this blinks...)!
- [ ] Improve the saving policy (this saves every time we update the properties so for card with lots of commands, this makes a lot of i/o)
...
- [-] Improve the request to the trello api to enforce the position inside trello (we may use the natural order of integer with position to determine such position)
