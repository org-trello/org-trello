...
- [ ] Use etags to avoid sync from action if it is not necessary [0%]
  - [ ] Exploit the etags from the card's property drawer and send get request with it
  - [ ] Retrieve etags from GET result and store it inside the card's property drawer
  - [ ] Improve sync-from-trello to avoid destroying the full content of the buffer before writing inside
...
* IN-PROGRESS 0.6.5 [57%]
- [X] Create backlog
- [X] Update version
- [X] Maintenance - Revert tests back to ert-deftest to have more explicit error outputs
- [X] Fix unit tests
- [ ] WTF?! - CI does not break when tests fail... Fix
- [ ] Long and multiline comments - https://github.com/org-trello/org-trello/issues/218
- [ ] Release notes
* DONE 0.6.4 [100%]
