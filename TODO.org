...
** TODO Improve the rendering of the webadmin page to add headers and action
** TODO Add a current scanning entry in the webadmin page
** TODO Add an action button to stop every running action
** TODO Release notes
** TODO Release
* TODO 0.1.6 [%]
** TODO POC: use a temporary buffer to sync with trello, when done, merge with the user's buffer.
** TODO Ensure sync-from-trello keep the order of the checklists clean
** TODO Now that elnode is here, this may be simpler to use a fake trello server (use clojure :D)
** TODO Improve the request to the trello api to enforce the position inside trello (we can use the natural order of integer with position to determine such position)
** TODO Filter out directly with the api the data we do not need to avoid unnecessary network traffic
** TODO Update the literate org-trello.org (completely desynchronized since 0.1.2) -> code something to avoid doing manual split
