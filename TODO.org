...
- [-] #256 - one user can have multiple trello accounts [2/3]
  - [X] Migrate the actual setup to the multi-account setup (transparent for user)
  - [X] Update install-key-and-token command to create the account setup
  - [ ] Make the install-key-and-token command be able to reuse existing setup
- [ ] Release notes
